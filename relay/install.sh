#!/usr/bin/env bash
# relay install script. Downloads latest relay release from fede-iglesias/tools and installs to /usr/local/bin/relay.
# Usage: curl -fsSL https://raw.githubusercontent.com/fede-iglesias/tools/main/relay/install.sh | bash
set -euo pipefail
...
[ -z "$TAG" ] && { echo "no relay release found in $REPO" >&2; exit 1; }

echo "installing relay v$TAG for $OS/$ARCH..."
